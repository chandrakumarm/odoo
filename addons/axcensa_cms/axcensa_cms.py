...
class college_standard(osv.Model):
    ''' Defining a standard related to college '''
    _description = 'College Standards'
    _name = 'college.standard'
    ...
    _columns = {
        'college_id': fields.many2one('college.college', 'College', required=True),
...
class college_college(osv.Model):
    
    ''' Defining College Information '''
...
#        Made field for Company Name 
        ...
        'com_name':fields.related('company_id', 'name', string="College Name", size=128, store=True, type="char"),
        'code': fields.char('Code', size=20, required=True, select=1),
        'state_id': fields.many2one("res.country.state", 'State'),
        'standards':fields.one2many('college.standard', 'college_id', 'Standards'),
...
class subject_subject(osv.Model):
    ...
    _columns = {
        ...
        'standard_ids':fields.many2many('college.standard', 'subject_standards_rel', 'standard_id', 'subject_id', 'Standards'),
...
#        'date_of_birth':fields.date('Birthdate', required=True, states={'done':[('readonly',True)]}),
        ...
        'previous_college_ids':fields.one2many('student.previous.college', 'previous_college_id', 'Previous College Detail', states={'done':[('readonly', True)]}),
        ...
        'college_id': fields.many2one('college.college', 'College', states={'done':[('readonly', True)]}),
...
#        'student_attachment_line' : fields.one2many('student.attachment', 'student_id', 'Attachment'),
        ...
        'division_id':fields.many2one('standard.division', 'Division', required=True,),
        'medium_id':fields.many2one('standard.medium', 'Medium', required=True,),
        'card_no':fields.char('Card No', size=12),
        'card_pin':fields.char('Pin No'),
        'topup_student_id': fields.char('Student Id'),
        'available_balance': fields.float('Balance Available'),
    ...
    def register_process(self, cr, uid, ids, context=None):
        view_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'axcensa_cms', 'view_card_register_form')[1]
        return {
            'name': "Card Registration",
            'view_mode': 'form',
            'view_id': view_id,
            'view_type': 'form',
            'res_model': 'card.register',                                       
    ...
    def top_up_process(self, cr, uid, ids, context=None):
        if context is None: context = {}
        context = dict(context, active_ids=ids, active_model=self._name)
        obj = self.browse(cr, uid, ids[0], context)
        context['default_student_id'] = obj.student_id.id
        context['default_topup_student_id'] = obj.pid
        context['default_division_id'] = obj.division_id.id
        context['default_card_no'] = obj.card_no
        context['default_available_balance'] = obj.available_balance

        view_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'axcensa_cms', 'view_top_up_form')[1]
        return {
            'name': "Card Recharge",
            'view_mode': 'form',
            'view_id': view_id,
    ...
    def action_topup(self, cr, uid, ids, context=None):
        obj = self.browse(cr, uid, ids[0], context)
        bal = int(obj.available_balance) + int(obj.amount)
        self.write(cr, uid, ids, {'available_balance': str(bal)})
        return True
    ...
    def admission_done(self, cr, uid, ids, context=None):
        college_standard_obj = self.pool.get('college.standard')
        for student_data in self.browse(cr, uid, ids, context=context):
            domain = [('standard_id', '=', student_data.standard_id.id)]
            college_standard_search_ids = college_standard_obj.search(cr, uid, domain, context=context)
            if not college_standard_search_ids:
                raise osv.except_osv(_('Warning'), _('The standard is not defined in a college'))
            domain = [('standard_id', '=', student_data.standard_id.id)]
            ...
            registation_code = str(student_data.college_id.state_id.name) + str('/') + str(student_data.college_id.city) + str('/') + str(student_data.college_id.name) + str('/') + str(reg_code)
            stu_code = self.pool.get('ir.sequence').get(cr, uid, 'student.code')
            student_code = str(student_data.college_id.code) + str('/') + str(student_data.year.code) + str('/') + str(stu_code)
...
class student_grn(osv.Model):
    ...
    def _grn_no(self, cr, uid, ids, name, args, context=None):
        ...
        for stud_grn in self.browse(cr, uid, ids, context=context):
            ...
            elif stud_grn['prefix'] == 'college':
                a = stud_grn.collegeprefix_id.code
            ...
            elif stud_grn['postfix'] == 'college':
                b = stud_grn.collegepostfix_id.code
                grn_no2 = grn_no1 + b
    ...
    _columns = {
        ...
        'prefix': fields.selection([('college', 'College Name'), ('year', 'Year'), ('month', 'Month'), ('static', 'Static String')], 'Prefix'),
        'collegeprefix_id': fields.many2one('college.college', 'College Name for Prefix'),
        'collegepostfix_id': fields.many2one('college.college', 'College Name for Suffix'),
        'postfix': fields.selection([('college', 'College Name'), ('year', 'Year'), ('month', 'Month'), ('static', 'Static String')], 'Suffix'),
...
class student_history(osv.Model):
    ...
    _columns = {
        ...
        'standard_id': fields.many2one('college.standard', 'Standard', required=True),
...
class student_previous_college(osv.Model):
    ''' Defining a student previous college information '''
    _name = "student.previous.college"
    _description = "Student Previous College"
    _columns = {
        'previous_college_id': fields.many2one('student.student', 'Student'),
        'name': fields.char('Name', size=64, required=True),
...
class academic_subject(osv.Model):
    ...
    _description = "Student Previous College"
    _columns = {
        'add_sub_id': fields.many2one('student.previous.college', 'Add Subjects', invisible=True),
        'name': fields.char('Name', size=64, required=True),
